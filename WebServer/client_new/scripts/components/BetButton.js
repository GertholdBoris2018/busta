            ...
            if(notPlayingOrBetting) {
                ...
                if(isBetting) {
                    ...
                    if (this.props.engine.gameState !== 'STARTING') {
                        btnContent.push(D.span({ key: 'bc-0'}, smallButton? '게임취소' : '게임취소'));
                        onClickFun = this.props.cancelBet;
                    } else {
						btnClasses += ' disable';
                        btnContent.push(D.span({ key: 'bc-0'}, '게임대기중'), D.a({ className: 'cancel', key: 'bc-1' }, D.br(),' (취소불가)'));
						btnClasses += ' cancel';
                    }

                    //Initial disable
                } else if(this.state.initialDisable) {
                	var btnText =   (canUserBet.message === 'Not enough bits')? (smallButton? '금액부족' : '배팅금액 부족') : (smallButton? '게임시작' : '게임시작');
                    ...
                    //Placing bet
                        ...
                        btnContent.push(D.span({ key: 'bc-4' }, smallButton? '게임시작' : '게임시작'));
                        btnClasses += ' disable unselect';

                    //Able to bet
                    } else {
                        btnContent.push(D.span({ key: 'bc-5' }, smallButton? '게임시작' : '게임시작'));
                        btnClasses += ' ';
            ...
            //The user is playing
                ...
                btnContent.push(
                    D.div({ className: 'btn-content', key: 'bc-6' },
                        D.span({ className: 'cashout-cont' }, '지금받기'),
