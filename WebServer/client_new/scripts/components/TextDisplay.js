            ...
            switch (Engine.gameState) {
                ...
                case 'ENDED':
                    content = D.div({ id: cId, className: 'ended', style: { fontSize: this.state.size.ended, color: 'red' } },
                        D.span({ className: 'busted' }, '게임결과'),
                        D.span({ className: 'at' }, 'X ' + Clib.formatDecimals(Engine.tableHistory[0].game_crash/100, 2) + 'x')
                    );
                    break;
                case 'STARTING':
                    var timeLeft = ((Engine.startTime - Date.now())/1000).toFixed(1);
                    content = D.div({ id: cId, className: '시작!', style: { fontSize: this.state.size.starting, color: 'grey' } },
                        D.span(null, '다음게임 ' + timeLeft + '초 전')
                    );
                    break;
