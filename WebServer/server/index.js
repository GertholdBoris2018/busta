...
app.use(function(req, res, next) {
    ...
    var ipAdd_incoming = req.connection.remoteAddress;
    if (ipAdd_incoming.substr(0, 7) == "::ffff:") {
        ipAdd_incoming = ipAdd_incoming.substr(7)
    }
    var ips = [];
    database.getBlockList(function(err, ipaddress) {
        for(var i=0; i<ipaddress.length; i++){
            ips.push(ipaddress[i].ipaddress);
        }
        if(ips.indexOf(ipAdd_incoming) !== -1){
            res.send(ipAdd_incoming + ' IP is not in whiteList');
        }
        else{
            var sessionId = req.cookies.id;
            database.getPopUpNotice(function(err,popup){
                if(err == "POPUP_DOES_NOT_EXISTS") req.popup_notice = "";
                else req.popup_notice = popup.name;
            });
            if (!sessionId) {
                var ipAddress = req.ip;
                var userAgent = req.get('user-agent');
                database.addAccessLog(ipAddress, userAgent, function (err) {
...
//var ips = ['45.23.34.23', '45.65.15.76'];
//get block ipaddress
// function request(){
//     var ip_blocks = database.getBlockList(function(err, ipaddress) {
//         ips = [];
//         for(var i=0; i<ipaddress.length; i++){
//             ips.push(ipaddress[i].ipaddress);
